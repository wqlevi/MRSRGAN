...
# - [ ] load pre-trained MedicalNet weight to FE, and freeze it during training
...
def train_loop(dataloader,model,loss_function,optimizer,device,epoch,num_epoch,checkpoint,model_name,writer):
    ...
    for i,imgs in enumerate(dataloader):
        ...
        with torch.no_grad(): 
            '''
            Write in training losses to log
            '''
        ...
        with torch.no_grad():
            psnr_v = utils.psnr(sr.squeeze().cpu().numpy(),imgs_hr.squeeze().cpu().numpy())
            ssim_v = utils.ssim(sr.squeeze().cpu().numpy(),imgs_hr.squeeze().cpu().numpy())
