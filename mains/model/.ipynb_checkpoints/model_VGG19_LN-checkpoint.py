...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 31 19:44:39 2022

- [ ] change FE to VGG19[:35] with first layer input_channel=1
- [ ] try load pre-trained weight to FE 
- [ ] spectral/ layer normalization to Discriminator(ref. real-ESRGAN+)
...
class DenseResidualBlock(nn.Module):
    ...
    def __init__(self, filters, res_scale=0.2):
        ...
        def block(in_features, non_linearity=True):
            ...
            if non_linearity:# BN added
                layers += [nn.BatchNorm3d(filters),nn.LeakyReLU()]
            return nn.Sequential(*layers)
...
class Generator(nn.Module): # interpolation scheme happens
    def __init__(self, channels=1, filters=64, num_res_blocks=1, num_upsample=1):
        ...
        self.res_blocks3 = nn.Sequential(*[ResidualInResidualDenseBlock(filters) for _ in range(num_res_blocks)])        # Second conv layer post residual blocks
...
class Discriminator(nn.Module):
    '''
    - [x] spectral norm added to each conv block
    '''
    def __init__(self):
